...
describe('Main features', () => {
  ...
  it('should fetch and display the global information from the server', async () => {
    ...
    await expect(element(by.id(GET_COUNTRY_NAME(globalSlug)))).toHaveText(
      '🌍 GLOBAL',
    );
    await expect(
      element(by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, i18n('CONFIRMED')))),
    ).toHaveText('4,977,471');
    await expect(
      element(by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, i18n('DEATHS')))),
    ).toHaveText('329,513');
    await expect(
      element(
        by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, i18n('RECOVERIES'))),
      ),
  ...
  it('should show countries screen after tapping the Countries tab, have Zimbabwe as the last element fetched', async () => {
    ...
    await expect(element(by.id(FLAT_COUNTRY_LIST))).toBeVisible();
    await element(by.id(FLAT_COUNTRY_LIST)).scrollTo('bottom');
  ...
  it('should open Country details screen on country selected', async () => {
    ...
    await expect(element(by.id(GET_COUNTRY_NAME('afghanistan')))).toHaveText(
      '🇦🇫 AFGHANISTAN',
    );
  });
});
