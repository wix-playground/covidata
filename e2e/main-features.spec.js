...
describe('Main features', () => {
  ...
  it('should fetch and display the global information from the server', async () => {
    ...
    await expect(
      element(by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, CONFIRMED))),
    ).toHaveText('4,977,471');
    await expect(
      element(by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, DEATHS))),
    ).toHaveText('329,513');
    await expect(
      element(by.id(GET_STAT_PREFIX_NAME_TOTAL(globalSlug, RECOVERIES))),
