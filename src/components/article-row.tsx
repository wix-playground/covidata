import {
  ...
  AnimatedImage,
  Assets,
} from '@wix/wix-react-native-ui-lib';
      ...
      <ListItem margin-15 center onPress={onPress} testID={TEST_ID_ARTICLE_ROW}>
        <View flex flexDirection={'row'}>
          ...
          <View marginL-15 marginB-15 flex flexDirection={'column'}>
            <Text uppercase text90H testID={TEST_ID_ARTICLE_SOURCE}>
              {article.source.name}
            </Text>
            <Text
              text80L
              marginV-10
              numberOfLines={1}
              testID={TEST_ID_ARTICLE_TITLE}>
              {article.title}
            </Text>
            <Text text100R testID={TEST_ID_ARTICLE_TIMESTAMP}>
