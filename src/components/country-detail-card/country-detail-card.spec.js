...
describe('Country card views', () => {
  ...
  it('correctly passes data to a card and constructs the necessary views', () => {
    ...
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_TOTAL(country.slug, i18n('CONFIRMED')),
        card,
      )[0].children[0],
    ).toEqual('1');
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_NEW(country.slug, i18n('CONFIRMED')),
        card,
      )[0].props.label,
    ).toEqual('+2');
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_TOTAL(country.slug, i18n('DEATHS')),
        card,
      )[0].children[0],
    ).toEqual('3');
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_NEW(country.slug, i18n('DEATHS')),
        card,
      )[0].props.label,
    ).toEqual('+4');
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_TOTAL(country.slug, i18n('RECOVERIES')),
        card,
      )[0].children[0],
    ).toEqual('5');
    expect(
      filterByTestID(
        GET_STAT_PREFIX_NAME_NEW(country.slug, i18n('RECOVERIES')),
        card,
      )[0].props.label,
    ).toEqual('+6');
  });
