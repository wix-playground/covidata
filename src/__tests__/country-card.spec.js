...
describe('Country card views', () => {
  it('should display the stat name, comma-separated value, new case badge in a single stat row', () => {
    ...
    expect(
      filterByTestID(TEST_ID_STAT_PREFIX_NAME(testIDPrefix, statName), row)[0]
        .children[0],
    ).toEqual(statName);

    expect(
      filterByTestID(
        TEST_ID_STAT_PREFIX_NAME_TOTAL(testIDPrefix, statName),
        row,
      )[0].children[0],
    ...
    expect(
      filterByTestID(
        TEST_ID_STAT_PREFIX_NAME_NEW(testIDPrefix, statName),
        row,
      )[0].props.label,
    ).toEqual('+' + statNew);
  });
  ...
  it('correctly passes data to a card and constructs the necessary views', () => {
    ...
    expect(
      filterByTestID(TEST_ID_COUNTRY_NAME(country.Slug), card)[0].children[0],
    ).toEqual('🇱🇹 Lithuania');
